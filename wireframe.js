...
function modalContent() {
  ...
  return `<div class='modal-overlay' data-close-modal='1'><div class='modal'><div class='modal-header'><div><h3>${isChatNode ? 'Добавить в чат' : 'Добавить в подразделение'}</h3><div class='muted'>Текущий контекст: ${node.name}</div></div><button data-close-modal='1'>✕</button></div><div class='entity-switcher'>${allowedAddTypes.map((key) => `<button class='${state.addType === key ? 'active' : ''}' data-add-type='${key}'>${modalTypeLabels[key]}</button>`).join('')}</div><div class='modal-form'>${form}</div><div class='modal-actions'><button data-close-modal='1'>Отмена</button><button data-submit-add='${state.addType}'>${submitLabels[state.addType]}</button></div></div></div>`;
}
