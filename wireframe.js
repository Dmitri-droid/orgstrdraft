...
const state = {
  node: 'root',
  tab: 'people',
  sel: { kind: 'node', id: 'root' },
  exp: { root: true, hq: true, feo: true },
  openTreeMenuNodeId: null,
  isCenterMenuOpen: false,
  isAddModalOpen: false,
  addType: 'employee',
  addContextNodeId: 'root',
};
...
function breadcrumb(id) {
  ...
  let cur = id;
  while (cur) {
    out.unshift(data.nodes[cur].name);
...
function openPrimaryChat(nodeId) {
  ...
  state.sel = { kind: 'chat', id: chat.id };
  state.tab = 'chats';
  toast(`Открыт основной чат: ${chat.name}`);
  render();
}
...
function renderTree(nodeId, level = 0) {
  const node = data.nodes[nodeId];
  ...
  if (hasChildren && expanded) {
    childrenHtml = node.children.map((id) => renderTree(id, level + 1)).join('');
  }

  const menu = menuOpen
    ? `<div class='tree-menu'>${(treeMenuByType[node.type] || treeMenuByType.department)
        .map((action) => `<button data-tree-action='${action}' data-node='${nodeId}'>${action}</button>`)
        .join('')}</div>`
    : '';

  return `
    <div class='tree-item-wrap'>
      <div class='tree-node' style='padding-left:${12 + level * 16}px'>
        ...
        <button class='tree-main-hit ${selected ? 'selected' : ''}' data-select-node='${nodeId}'>
          ...
          <span>
            <div>${node.name}</div>
            <div class='tree-type'>${node.typeLabel}</div>
          </span>
        </button>
...
function list(items, kind, template) {
  if (!items.length) return `<div class='empty'>Пусто для выбранного узла.</div>`;
  return items
    .map((item) => `<div class='list-row ${state.sel.kind === kind && state.sel.id === item.id ? 'selected' : ''}' data-k='${kind}' data-id='${item.id}'>${template(item)}</div>`)
    .join('');
}
...
function centerContent() {
  ...
  let content = '';
  if (state.tab === 'people') {
    content = `<div class='row-actions'><input placeholder='Поиск сотрудника'/><button>Фильтр</button><button>Сортировка</button></div>` +
      list(people, 'employee', (x) => `<div class='avatar'>${x.name.split(' ').map((v) => v[0]).join('')}</div><div class='grow'><b>${x.name}</b><div>${x.pos}</div><small>${x.sub}</small></div><button data-msg='${x.name}'>Написать</button>`);
  }
  if (state.tab === 'positions') {
    content = `<div class='row-actions'><input placeholder='Поиск должности'/><button>Все</button><button>Занятые</button><button>Вакантные</button></div>` +
      list(positions, 'position', (x) => `<div class='grow'><b>${x.title}</b><div>${x.status} · ${x.assignee}</div></div><button>${x.status === 'Вакантна' ? 'Назначить' : 'Открыть'}</button>`);
  }
  if (state.tab === 'chats') {
    content = list(chats, 'chat', (x) => `<div class='grow'><b>${x.name}</b><div>${x.type} · ${x.participants} участников</div><small>${x.last}</small></div><button data-open-chat='${x.id}'>Открыть</button>`);
  }
  if (state.tab === 'files') {
    content = list(files, 'file', (x) => `<div class='grow'><b>${x.name}</b><div>${x.type} · ${x.owner}</div></div><button data-open-file='${x.id}'>Открыть</button>`);
  }
  if (state.tab === 'about') {
    content = `<div class='card'><p><b>Название:</b> ${node.name}</p><p><b>Тип:</b> ${node.typeLabel}</p><p><b>Руководитель:</b> ${node.leader}</p><p><b>Описание:</b> ${node.desc}</p></div>`;
  }

  const centerMenu = state.isCenterMenuOpen
    ? `<div class='center-menu'>${(centerMenuByType[node.type] || centerMenuByType.department)
        .map((action) => `<button data-center-action='${action}'>${action}</button>`)
        .join('')}</div>`
    : '';
  ...
  return `
    <div class='header'>
      <div>
        <div class='muted'>${breadcrumb(state.node)}</div>
        <h2>${node.name}</h2>
        <div class='muted'>${node.summary}</div>
      </div>
      <div class='row-actions'>
        <button data-primary-chat='${state.node}'>Открыть чат</button>
        <button data-open-add='${state.node}'>Добавить</button>
        <div class='menu-anchor'>
          <button data-open-center-menu='1'>Еще</button>
          ${centerMenu}
        </div>
      </div>
    </div>
    <div class='tabs'>
      ${[['people', 'Люди'], ['positions', 'Должности'], ['chats', 'Чаты'], ['files', 'Файлы'], ['about', 'О подразделении']]
        .map(([k, l]) => `<button class='${state.tab === k ? 'active' : ''}' data-tab='${k}'>${l}</button>`).join('')}
    </div>
    ${content}
  `;
}
...
function detailsContent() {
  ...
  if (s.kind === 'employee') {
    const e = data.people.find((x) => x.id === s.id);
    if (!e) return '';
    return `<div class='card'><h3>${e.name}</h3><p>${e.pos}</p><p>Статус: ${e.status}</p><div class='row-actions'><button data-msg='${e.name}'>Написать</button><button>Позвонить</button></div></div>`;
  }
  if (s.kind === 'position') {
    const p = data.positions.find((x) => x.id === s.id);
    if (!p) return '';
    return `<div class='card'><h3>${p.title}</h3><p>${p.status}</p><p>${p.assignee}</p><button>Назначить сотрудника</button></div>`;
  }
  if (s.kind === 'chat') {
    const c = data.chats.find((x) => x.id === s.id);
    if (!c) return '';
    return `<div class='card'><h3>${c.name}</h3><p>${c.type}</p><button data-open-chat='${c.id}'>Открыть</button></div>`;
  }
  if (s.kind === 'file') {
    const f = data.files.find((x) => x.id === s.id);
    if (!f) return '';
    return `<div class='card'><h3>${f.name}</h3><p>${f.type}</p><button data-open-file='${f.id}'>Открыть</button></div>`;
  }
...
function modalContent() {
  ...
  let form = '';
  if (state.addType === 'employee') {
    form = `<label>Имя и фамилия <input placeholder='Анна Смирнова'/></label><label>Должность <input placeholder='Аналитик'/></label><label>Роль / функция <input placeholder='Финансы'/></label><label>Руководитель <select><option>Иван Демьянов</option><option>Лариса Иванова</option></select></label><label>Email / логин (optional) <input placeholder='anna@neurocom.ru'/></label>`;
  }
  if (state.addType === 'position') {
    form = `<label>Название должности <input placeholder='Координатор'/></label><label>Тип <select><option>Обычная</option><option>Руководящая</option><option>Вакансия</option></select></label><label>Кому подчиняется <input placeholder='Руководитель отдела'/></label><label>Описание <textarea placeholder='Краткое описание'></textarea></label><label>Статус <select><option>Занята</option><option>Вакантна</option></select></label>`;
  }
  if (state.addType === 'department') {
    form = `<label>Название подразделения <input placeholder='Группа контроля'/></label><label>Тип <select><option>Департамент</option><option>Отдел</option><option>Группа</option></select></label><label>Родительский узел <input value='${node.name}'/></label><label>Руководитель (optional) <input placeholder='ФИО'/></label><label>Описание <textarea placeholder='Описание'></textarea></label>`;
  }
  if (state.addType === 'chat') {
    form = `<label>Название чата <input placeholder='Планерка отдела'/></label><label>Тип чата <select><option>Общий</option><option>Служебный</option><option>Планерка</option></select></label><label><input type='checkbox'/> Сделать основным чатом подразделения</label><label>Описание <textarea placeholder='Заметка'></textarea></label>`;
  }
  if (state.addType === 'file') {
    form = `<label>Название файла <input placeholder='Регламент.pdf'/></label><label>Тип файла <input placeholder='PDF / DOCX'/></label><label>Описание <textarea placeholder='Описание'></textarea></label><label>Загрузка файла <div class='upload-placeholder'>Dropzone placeholder</div></label>`;
  }
    ...
    <div class='modal-overlay' data-close-modal='1'>
      <div class='modal'>
        <div class='modal-header'>
          <div><h3>Добавить в подразделение</h3><div class='muted'>Текущий контекст: ${node.name}</div></div>
          <button data-close-modal='1'>✕</button>
        </div>
        <div class='entity-switcher'>
          ${Object.entries(addTypes).map(([k, v]) => `<button class='${state.addType === k ? 'active' : ''}' data-add-type='${k}'>${v}</button>`).join('')}
        </div>
        <div class='modal-form'>${form}</div>
        <div class='modal-actions'><button data-close-modal='1'>Отмена</button><button data-submit-add='${state.addType}'>${submitLabels[state.addType]}</button></div>
      </div>
    </div>
  `;
}
...
function render() {
  app.innerHTML = `
    <div class='top'>
      ...
      <span class='muted'>${breadcrumb(state.node)}</span>
      <input class='grow' placeholder='Глобальный поиск'/>
      <button class='active'>Компания</button><button>Люди</button><button>Чаты</button><button>Файлы</button>
      <button data-open-add='${state.node}'>Создать</button><button>Еще</button>
    </div>
    <div class='layout'>
      <div class='panel left'>
        <h3>Оргструктура</h3>
        <input placeholder='Поиск в структуре'/>
        <div class='chips'><button class='active'>Все</button><button>Подразделения</button><button>Люди</button><button>Должности</button><button>Чаты</button><button>Вакансии</button></div>
        ${renderTree('root')}
      </div>
      <div class='panel center'>${centerContent()}</div>
      <div class='panel right'>${detailsContent()}</div>
    </div>
    ${modalContent()}
  `;
...
function bindInteractions() {
  app.querySelectorAll('[data-chevron]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const id = btn.dataset.chevron;
      state.exp[id] = !state.exp[id];
      state.openTreeMenuNodeId = null;
      render();
    };
  });

  app.querySelectorAll('[data-select-node]').forEach((btn) => {
    btn.onclick = () => {
  ...
  app.querySelectorAll('[data-open-tree-menu]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const id = btn.dataset.openTreeMenu;
      state.openTreeMenuNodeId = state.openTreeMenuNodeId === id ? null : id;
      state.isCenterMenuOpen = false;
      render();
    };
  });

  app.querySelectorAll('[data-tree-action]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const action = btn.dataset.treeAction;
      const nodeId = btn.dataset.node;
      if (action.includes('Добавить')) {
        openAddModal(nodeId, action.includes('сотрудника') ? 'employee' : 'department');
        return;
      }
      if (action.includes('Создать чат')) {
        openAddModal(nodeId, 'chat');
        return;
      }
      if (action.startsWith('Открыть')) {
        state.node = nodeId;
        state.sel = { kind: 'node', id: nodeId };
      }
      state.openTreeMenuNodeId = null;
      toast(`${action}: ${data.nodes[nodeId].name}`);
      render();
    };
  });

  app.querySelectorAll('[data-open-center-menu]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      state.isCenterMenuOpen = !state.isCenterMenuOpen;
      state.openTreeMenuNodeId = null;
      render();
    };
  });

  app.querySelectorAll('[data-center-action]').forEach((btn) => {
    btn.onclick = () => {
      const action = btn.dataset.centerAction;
      state.isCenterMenuOpen = false;
      if (action === 'Открыть чат') return openPrimaryChat(state.node);
      toast(`${action}: ${data.nodes[state.node].name}`);
      render();
    };
  });
  ...
  app.querySelectorAll('.list-row').forEach((row) => {
    row.onclick = () => {
      state.sel = { kind: row.dataset.k, id: row.dataset.id };
      render();
    };
  });

  app.querySelectorAll('[data-msg]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      toast(`Написать: ${btn.dataset.msg}`);
    };
  });

  app.querySelectorAll('[data-primary-chat]').forEach((btn) => {
    btn.onclick = () => openPrimaryChat(btn.dataset.primaryChat);
  });

  app.querySelectorAll('[data-open-chat]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      const chat = data.chats.find((c) => c.id === btn.dataset.openChat);
      if (chat) toast(`Открыть чат: ${chat.name}`);
    };
  });
  ...
  app.querySelectorAll('[data-open-file]').forEach((btn) => {
    btn.onclick = (e) => {
      e.stopPropagation();
      toast(`Открыть файл: ${btn.dataset.openFile}`);
    };
  });

  app.querySelectorAll('[data-open-add]').forEach((btn) => {
    btn.onclick = () => openAddModal(btn.dataset.openAdd, 'employee');
  });

  app.querySelectorAll('[data-add-type]').forEach((btn) => {
    btn.onclick = () => {
      state.addType = btn.dataset.addType;
      render();
    };
  });
  ...
  app.querySelectorAll('[data-close-modal]').forEach((btn) => {
    btn.onclick = (e) => {
      if (e.target === e.currentTarget || btn.dataset.closeModal === '1') {
  ...
  app.querySelectorAll('[data-submit-add]').forEach((btn) => {
    btn.onclick = () => {
      state.isAddModalOpen = false;
